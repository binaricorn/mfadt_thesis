$(document).ready(function() {
    ...
    var s = ["We are Taylorhealth.", "You spend almost three fourths of your waking hours at work.", "That's a lot of hours, Don't you want that time to be really aawesome??", "Well, we do, Making sure you're happy is our job, We want you to feel like your workplace is Taylored just for you.", "Your only complaint will be that your face hurts from smiling too much!", "Yup, we're just that awesome, Lucky for you, we're hiring! Help us make you happy! Mind and body!", "Sign up for a quick interview, we'd really like to get to know you!"];
    var sc = [];
    ...
    for (i = 0; i < s.length; i++) {
        sc[i] = new SpeechSynthesisUtterance(s[i]);
    ...
    function loop(count) {        
        if (count < s.length-1) {
            count++;
            sc[count].voice = speechSynthesis.getVoices().filter(function(voice) { return voice.name == "Google UK English Female"; })[0];
            speechSynthesis.speak(sc[count]);
            sc[count].onend = function(event) {
